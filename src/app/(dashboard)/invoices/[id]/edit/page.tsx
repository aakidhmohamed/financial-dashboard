...
export default function EditInvoicePage() {
    ...
    useEffect(() => {
        fetch(`/api/invoices/${invoiceId}`)
            ...
            .then((data: Invoice) => {
                ...
                setAdvancePaid(data.advance_paid)
                setRemarks(data.remarks || '')
                setItems(data.items.length > 0 ? data.items : [{ description: '', quantity: 1, unit_price: 0 }])
                setLoading(false)
            })
    ...
    useEffect(() => {
        fetch('/api/accounts')
            ...
            .then(d => {
                ...
                // Auto-select Cash or Bank, otherwise first asset
                if (!paymentAccountId) {
                    const cashAccount = accountList.find((a: Account) => a.name.toLowerCase().includes('cash'))
                    const bankAccount = accountList.find((a: Account) => a.name.toLowerCase().includes('bank'))
                    const anyAsset = accountList.find((a: Account) => a.type === 'asset')

                    const defaultAccount = cashAccount || bankAccount || anyAsset
    ...
    const handleSubmit = async (e: React.FormEvent | null, statusOverride?: string, advanceOverride?: number) => {
        ...
        // Validation: Advance payment requires account
        ...
        if ((finalAdvance > 0 || isPaidStatus) && !paymentAccountId) {
            alert('Please select a payment account for the advance payment')
            return
        }
        ...
        try {
            const body = {
                ...
                status: finalStatus, // Include status update
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-extrabold text-foreground">
                        Edit {invoice.document_type === 'quotation' ? 'Quotation' : 'Invoice'}
                    </h1>
            ...
            <form onSubmit={(e) => handleSubmit(e)} className="space-y-6">
                    ...
                    <div className="bg-card border border-border rounded-2xl p-6 space-y-3">
                        ...
                        <div className="flex justify-between items-center border-t-2 border-foreground pt-3">
                            <span className="font-bold text-lg">
                                {(invoice.status === 'paid' || balance <= 0) ? 'STATUS' : 'BALANCE'}
                            </span>
                            <span className={`font-black text-xl ${(invoice.status === 'paid' || balance <= 0) ? 'text-green-600' : ''}`}>
                                {(invoice.status === 'paid' || balance <= 0) ? 'PAID' : formatCurrency(balance)}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-end gap-3">
                    <Link
                        ...
                        className="px-8 py-3 bg-secondary text-foreground rounded-xl font-semibold text-sm hover:bg-muted transition-all"
                    >
                        Cancel
                    </Link>
